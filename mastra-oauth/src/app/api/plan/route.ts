...
export async function POST(request: NextRequest) {
  try {
    ...
    const { attractions, accessToken } = body as {
    ...
    for (const attraction of attractions) {
      try {
        const location = await geocodeLocation(attraction.address, accessToken);
        locations.push({
    ...
    const route = await calculateRoute(coordinates, accessToken);
