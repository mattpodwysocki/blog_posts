...
// Create Mastra tools
const searchLocationTool = createTool({
  id: 'search-location',
...
const searchCategoryTool = createTool({
  id: 'search-category',
  description:
  ...
  execute: async ({ category, near }) => {
    ...
    const nearLocation = await geocodeLocation(near);
    const results = await categorySearch(category, {
...
const planRouteTool = createTool({
  id: 'plan-route',
...
export async function processNaturalLanguageQuery(query: string): Promise<AgentResponse> {
  // Clear captured data
